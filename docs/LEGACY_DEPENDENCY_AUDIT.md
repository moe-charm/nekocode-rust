...
現時点では依存監査、移行前commit `c4bb63d` の `legacy-2026-pre-rust-first` タグ固定、旧build/release/PR workflowのmanual-only切替、コミット済みRust-first HEADのclean checkout（core 9 tests、CLI check/index/context、MCP smoke）まで完了。5-binaryのCargo/build/setup/Docker導線とsecurityのroot監査はまだlegacy依存があるため、物理移動はその切替後に実施する。
