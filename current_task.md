...
- Phase 4（MCP hardening・legacy archive）: 進行中（CLIを2コマンドへ縮小し、Cargo/Makeの一般導線をcanonical defaultへ変更。coreの旧session/config APIと依存をnon-default `legacy` featureへ隔離。物理archive前のfinal tag/read-only archiveは未実施）
