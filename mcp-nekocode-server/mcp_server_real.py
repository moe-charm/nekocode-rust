...
class NekoCodeMCPServer:
    ...
    def _define_tools(self) -> List[Dict]:
        ...
        return [
            ...
            {
                "name": "enforce_refactor_mode",
                "description": "🛡️ 保護モード: 直接編集を抑止（ファイルを一時的に読み取り専用に）",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "dry_run": {"type": "boolean", "default": False, "description": "実行せず影響対象のみ表示"}
                    }
                }
            },
            {
                "name": "disable_refactor_mode",
                "description": "🔓 保護解除: 書き込み可能に戻す",
                "inputSchema": {"type": "object", "properties": {}}
            },
            {
                "name": "setup_precommit_guard",
                "description": "🚦 pre-commitガード: 危険なスタブ改変を検出してコミットを拒否",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "patterns": {"type": "array", "items": {"type": "string"}, "description": "拒否パターン（正規表現）"}
                    }
                }
            }
        ]
    ...
    async def _tool_enforce_refactor_mode(self, args: Dict) -> Dict:
        """プロジェクト配下を読み取り専用にし、直接編集を抑止"""
        dry = bool(args.get('dry_run', False))
        root = os.getcwd()
        skipped_dirs = {'.git', 'target', 'releases', 'node_modules', '.venv', '.m2'}
        affected = []
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for fname in filenames:
                ...
                try:
                    st = os.stat(fpath)
                    # if writable, plan to remove write bits
                    if (st.st_mode & 0o222) != 0:
                        affected.append(fpath)
                        if not dry:
                            os.chmod(fpath, st.st_mode & ~0o222)
                except Exception as e:
                    logger.debug(f"chmod skip {fpath}: {e}")
        msg = f"🔒 Protected {len(affected)} files (dry-run={dry})."
        return {"content": [{"type": "text", "text": msg}]}

    async def _tool_disable_refactor_mode(self, args: Dict) -> Dict:
        """読み取り専用を解除（ユーザ書き込み可）"""
        root = os.getcwd()
        skipped_dirs = {'.git', 'target', 'releases', 'node_modules', '.venv', '.m2'}
        count = 0
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for fname in filenames:
                ...
                try:
                    st = os.stat(fpath)
                    # ensure user write bit set
                    new_mode = st.st_mode | 0o200
                    if new_mode != st.st_mode:
                        os.chmod(fpath, new_mode)
                        count += 1
                except Exception as e:
                    logger.debug(f"chmod restore skip {fpath}: {e}")
        return {"content": [{"type": "text", "text": f"🔓 Unprotected {count} files"}]}

    async def _tool_setup_precommit_guard(self, args: Dict) -> Dict:
        """pre-commitフックを配置し、危険なスタブ改変を拒否"""
        patterns = args.get('patterns') or [
            r'unimplemented!\(', r'panic!\(', r'TODO[: ]', r'FIXME[: ]',
            r"throw new Error\('TODO", r'pass\s+#\s*stub', r'#\s*stub', r'NOT_IMPLEMENTED'
        ]
        hook_dir = os.path.join(os.getcwd(), '.git', 'hooks')
        if not os.path.isdir(hook_dir):
            return {"content": [{"type": "text", "text": "このディレクトリはGitリポジトリではありません（.git/hooks 不在）"}], "isError": True}
        hook_path = os.path.join(hook_dir, 'pre-commit')
        script = [
            "#!/usr/bin/env bash",
            "set -euo pipefail",
            "# Auto-generated by NekoCode MCP: pre-commit guard",
            "PATTERNS=(" + ' '.join([f"'{p}'" for p in patterns]) + ")",
            "FILES=$(git diff --cached --name-only --diff-filter=ACM | tr '\n' ' ')",
            "if [ -z \"$FILES\" ]; then exit 0; fi",
            "FAILED=0",
            "for p in \"${PATTERNS[@]}\"; do",
            "  if git grep -n -E --cached -- $p -- $FILES >/dev/null 2>&1; then",
            "    echo '🚫 pre-commit: Forbidden pattern detected:' $p >&2",
            "    git grep -n -E --cached -- $p -- $FILES | sed 's/^/  /' >&2",
            "    FAILED=1",
            "  fi",
            "done",
            "if [ $FAILED -eq 1 ]; then",
            "  echo 'Use nekorefactor MCP tools (replace/insert/movelines/moveclass, smart_*) instead of stubbing.' >&2",
            "  exit 1",
            "fi",
            "exit 0"
        ]
        try:
            ...
            return {"content": [{"type": "text", "text": f"✅ pre-commit guard installed at {hook_path}"}]}
        except Exception as e:
            return {"content": [{"type": "text", "text": f"フック設置に失敗: {e}"}], "isError": True}
    # MCPプロトコル実装
    # ========================================
